...
from libs.cmcc.cmcc_process import CmccProcess
...
class MainProcess:
    ...
    def check_order_param(self, text):
        ...
        result = {'order_name':order, 'param_count':need_count, 'system': ORDER_DIC.get(order).get('system'), 'actual_order':ORDER_DIC.get(order).get('actual_order')}
